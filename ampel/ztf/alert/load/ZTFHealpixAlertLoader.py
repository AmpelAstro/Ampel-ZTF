...
class ZTFHealpixAlertLoader(AbsAlertLoader):
    ...
    def query_ampel_api(self) -> List[Dict[Any, Any]]:
        query_res = []
        jd = Time(self.time, scale="utc").jd
        query_url = "alerts/healpix/skymap"
        params = {
            "nside": self.nside,
            "pixels": self.healpix,
            "with_history": "false",
        ...
        self.logger.debug(
            f"Querying {len(self.healpix)} ipix {self.healpix[0:10]}..."
        )

        response = self.session.post(query_url, json=params)
        response.raise_for_status()
        try:
        ...
        if len(query_res) == self.chunk_size:
            self.logger.warn(
                f"Query result was limited by chunk size={self.chunk_size}!"
            )
        elif response.json()["chunks_remaining"] != 0:
            self.logger.warn("Query has leftover chunks!")
        self.logger.debug(f"Got {len(query_res)} results")
        return query_res
